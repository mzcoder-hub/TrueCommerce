...
import { listOrdersUnPaid, sendCancelRequest } from '../../actions/orderActions'
...
const UnPaidScreen = ({ history }) => {
  ...
  const { loading, list } = orderNotPaid

  useEffect(() => {
    if (!userInfo) {
      history.push('/login')
    } else {
      dispatch(listOrdersUnPaid(userInfo._id))
    }
            ...
            <Card style={{ marginTop: 5 }} key={key}>
              ...
              <CardContent>
                ...
                <Grid container spacing={2} key='bottomCard'>
                  <Grid item xs={8} key={`OrderRating$key`}></Grid>
                  <Grid item xs={4} key={`ButtonReorder$key`}>
                    {val.isCanceled ? (
                      <></>
                    ) : (
                      <Button
